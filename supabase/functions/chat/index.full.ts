...
const ENCRYPTION_KEY = Deno.env.get('AI_API_ENCRYPTION_KEY');
...
console.log('ENCRYPTION_KEY is set and valid length.');
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Request body parsed:', { messages, service_config, action, service_data });
    ...
    if (action === 'save_ai_service_settings' && service_data) {
      ...
      console.log('Processing save_ai_service_settings action.');
      console.log('Service data received:', service_data);
      ...
      if (api_key) { // Only encrypt and update if API key is provided
        console.log('API key provided. Encrypting...');
        try {
          ...
          console.log('API key encrypted and added to upsertData.');
        } catch (e: any) {
        ...
        console.log('No new API key provided for existing service, skipping key update.');
      }
    ...
    console.log('Messages structure validated');
    ...
    console.log(`Fetching AI service settings for ID: ${service_config.id} and user: ${user.id}`);
    ...
    console.log('AI service settings fetched successfully.');
    ...
    console.log('Using model:', model, 'for service type:', aiService.service_type);
    ...
    console.log('User Messages (filtered):', userMessages);
    ...
    console.log('Cleaned System Prompt (for Google):', cleanSystemPrompt);
    ...
    switch (aiService.service_type) {
      ...
      case 'google':
        console.log('Processing for Google AI (multimodal)');
        ...
        const googleBody: any = {
            ...
            if (typeof msg.content === 'string') {
              ...
              parts = msg.content.map((part: any) => {
                 if (part.type === 'text') {
                   ...
                   console.log('Processing image part');
                   // Google AI expects image data in a specific format
                   // The URL should be a data URL (Base64)
                   try {
                     const [mimeTypePart, base64DataPart] = part.image_url.url.split(';base64,');
                     if (!mimeTypePart || !base64DataPart) {
                       console.error('Invalid data URL format for image part');
                       return null; // Skip invalid image part
                     }
                     // Extract mime type between 'data:' and ';'
                     const mimeType = part.image_url.url.substring(part.image_url.url.indexOf(':') + 1, part.image_url.url.indexOf(';'));
                     const base64Data = base64DataPart;
                     console.log('Extracted mime type:', mimeType);
                     console.log('Extracted base64 data length:', base64Data.length); // Log length instead of content
                     return {
                       inline_data: {
                         mime_type: mimeType,
                         data: base64Data
          ...
          }).filter((content: any) => content.parts.length > 0), // Filter out messages with no valid parts
        };
        console.log('Google API request body:', JSON.stringify(googleBody, null, 2));
    ...
    console.log(`Successfully processed ${aiService.service_type} request`);
