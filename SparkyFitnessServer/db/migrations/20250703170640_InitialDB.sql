...
-- Create a placeholder for auth.users table
...
BEGIN
  ...
  IF target_user_id = current_setting('app.user_id')::uuid THEN
    RETURN true;
  END IF;
  ...
  RETURN EXISTS (
    ...
    WHERE fa.family_user_id = current_setting('app.user_id')::uuid
