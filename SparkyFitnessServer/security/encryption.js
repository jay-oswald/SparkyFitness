...
if (ENCRYPTION_KEY_RAW.length !== 64 && ENCRYPTION_KEY_RAW.length !== 44) {
  log('error', `SPARKY_FITNESS_API_ENCRYPTION_KEY has an invalid length. Expected 64 hex characters or 44 Base64 characters, got ${ENCRYPTION_KEY_RAW.length}.`);
  process.exit(1);
}

if (JWT_SECRET_RAW.length !== 64 && JWT_SECRET_RAW.length !== 44) {
  log('error', `JWT_SECRET has an invalid length. Expected 64 hex characters or 44 Base64 characters, got ${JWT_SECRET_RAW.length}.`);
  process.exit(1);
}
